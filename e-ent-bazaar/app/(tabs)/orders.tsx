...
import { getCustomerOrdersWithDetails, OrderWithDetails, OrderStatus } from '../../lib/orderService';
...
export default function OrdersScreen() {
    ...
    useEffect(() => {
        if (isGuest) {
            Alert.alert('Guest Mode', 'Please sign in to view your orders and access all features.');
    ...
    const loadOrders = async () => {
        ...
        try {
            const customerOrders = await getCustomerOrdersWithDetails(user.id);
    ...
    useFocusEffect(
        React.useCallback(() => {
            console.log('Orders screen focused - fetching fresh data');
            loadOrders();
        }, [user?.id])
    );
    ...
    useEffect(() => {
        console.log('Orders - Current language:', i18n.language);
        console.log('Orders - Test translation:', t('orders.myOrders'));
    ...
    const getStatusText = (status: string) => {
        switch (status) {
            ...
            default:
                return status ? status.charAt(0).toUpperCase() + status.slice(1) : t('orders.unknownStatus');
        }
    };
    ...
    const renderOrderItem = ({ item }: { item: OrderWithDetails }) => (
        ...
        >
            ...
            <View style={styles.orderDetails}>
                ...
                <Text style={styles.manufacturerName}>{item.manufacturer_name}</Text>

                <View style={styles.orderInfo}>
                    <Text style={styles.orderInfoText}>{t('orders.qty')}: {item.quantity}</Text>
                    <Text style={styles.orderInfoText}>₹{item.price}/unit</Text>
                </View>
    ...
    const renderStatusFilter = ({ item }: { item: { label: string; value: OrderStatus | 'all' } }) => (
        <TouchableOpacity
            style={[
                styles.statusFilter,
        ...
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.subtitle}>
                    {filteredOrders.length} {t('orders.order', { count: filteredOrders.length })}
                </Text>
            </View>
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyState}>
                        <Text style={styles.emptyStateText}>{t('orders.noOrdersFound')}</Text>
